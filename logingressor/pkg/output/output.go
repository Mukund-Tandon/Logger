...
func doBatchInsert(logbatch models.Logbatch, conn clickhouse.Conn) {
	...
	for i := 0; i < logBatchSize; i++ {
		...
		fmt.Println(timestamp)
		clickHouseFormatTimeStamp := timestamp.Format("2006-01-02 15:04:05.000000")

		fmt.Printf("Formatted timestamp: %s\n", clickHouseFormatTimeStamp)

		// Convert to UTC if not already
		utcTimestamp := timestamp.UTC()
		fmt.Printf("UTC timestamp: %s\n", utcTimestamp.Format("2006-01-02 15:04:05.000000"))

		// Format for ClickHouse insert
		clickHouseInsertFormat := utcTimestamp.Format("2006-01-02 15:04:05.000000")
		fmt.Printf("ClickHouse insert format: %s\n", clickHouseInsertFormat)
